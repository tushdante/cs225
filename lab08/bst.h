...
class BST {
	...
	//helpers for remove implementation
	void doRemoval(Node * & croot);
	void noChildRemove(Node * & croot);
	void oneChildRemove(Node * & croot);
	void twoChildRemove(Node * & croot);
	Node * & IOP(Node * & croot);
	Node * & rightMostChild(Node * & croot);
        vector<V> keySort(Node * & croot) const;
	vector<V> inorderTraversal(Node * & croot, vector<V> &myVector) const;
	void levelOrder(Node * croot, vector<V> & myVector) const;
};
