...
template <typename Etype>
void List<Etype>::printForward() const
{
   ...
   else
   {
      ...
      cout << ">\n";
      if (i == 100)
         cout << "Loop detected; printing aborted prematurely.\n";
   }
}
...
// printBackward
//    - prints out the values in the list, in order by position
//        from last to first position. Assumes existence of
//        operator<< for generic type.
template <typename Etype>
void List<Etype>::printBackward() const
{
   typename List<Etype>::ListNode* outPtr = tail;
...
template <typename Etype>
void List<Etype>::printSize() const
{
   cout << "List size: " << size << "\n";
}
...
// printAll
//    - prints the list size, then prints the list forward
//      and backward.
template <typename Etype>
void List<Etype>::printAll() const
{
   printSize();
   printForward();
   printBackward();  
