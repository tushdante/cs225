...
#define TESTIMAGE1 "simple.bmp"
#define TESTIMAGE2 "simple2.bmp"
...
bool BMPdiffWithTolerance(string filename, BMP img, int tolerance) {
  BMP soln_img;
    ...
    if(anim.size() > 0) {
      for(int i = 0; i < anim.size(); i++) {
        if(argc > 1) // If you are generating the solns
          anim.frame(i).WriteToFile(("soln_" + filename + "_frame" + itos(i) + ".bmp").c_str());

        if(BMPdiffWithTolerance("soln_" + filename + "_frame" + itos(i) + ".bmp", anim.frame(i), IMAGETOLERANCE))
