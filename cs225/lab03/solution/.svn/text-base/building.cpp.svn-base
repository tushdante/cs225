...
// Building
//  - Copy Constructor
//  - Parameters - copyObj: Building to make an exact copy of
//  - Creates a building that is identical to copyObj with its own
//    dynamic memory
Building::Building(Building const & copyObj)
{
   copy(copyObj);
}

// copy
//  - helper function to make a deep copy of a Building object
void Building::copy(Building const & copyObj)
{
   mySize = copyObj.mySize;
   myApartments = new Apartment*[mySize];
   for(int x=0; x<mySize; x++)
   {
      if(copyObj.myApartments[x] != NULL)
         myApartments[x] = new Apartment(*(copyObj.myApartments[x]));
      else
         myApartments[x] = NULL;
   }
}
...
// clear
//  - helper function to free all resources used by the Building instance
void Building::clear()
{
   for(int x=0; x<mySize; x++)
   {
      if(myApartments[x] != NULL)
         delete myApartments[x];
   }
   delete [] myApartments;
}
...
Building const & Building::operator=(Building const & rhs)
{
   if(this != &rhs)
   {
      clear();
...
{
   if(index < 0 || index >= mySize || myApartments[index] == NULL)
      return false;
