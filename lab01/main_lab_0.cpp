...
BMP makeImageSepia(BMP original, int x, int y, int width, int height)
{
    for (int i = x; i < x + width; i++)
    {
        ...
        {
            int red = min(255,(int) (	((int) original(i, j)->Red * 0.393)+ 
					((int) original(i, j)->Green *.769)+
					((int) original(i, j)->Blue * .189)) );

            int green = min(255, (int)( ((int) original(i, j)->Red * .349)+ 
					((int) original(i, j)->Green *.686)+
					((int) original(i, j)->Blue * .168)) );

            int blue  = min(255,(int) ( ((int) original(i, j)->Red * 0.272)+ 
					((int) original(i, j)->Green *.534)+
					((int) original(i, j)->Blue * .131)) );
            original(i, j)->Red = red;
            original(i, j)->Green = green;
            original(i, j)->Blue = blue;
        }
    }
...
{
    for (int j = y; j < y + height; j=j+2)
    {
        for (int i = x; i < x + width; i=i+2)
        {
            int red = min(255,(int) (	(((int) original(i, j)->Red)+ 
					((int) original(i+1, j)->Red)+
					((int) original(i, j+1)->Red))+
					((int) original(i+1, j+1)->Red))/4 );

            int blue = min(255,(int) (	(((int) original(i, j)->Blue)+ 
					((int) original(i+1, j)->Blue)+
					((int) original(i, j+1)->Blue))+
					((int) original(i+1, j+1)->Blue))/4 );


            int green  = min(255,(int) ((((int) original(i, j)->Green)+ 
					((int) original(i+1, j)->Green)+
					((int) original(i, j+1)->Green))+
					((int) original(i+1, j+1)->Green))/4 );

            original(i, j)->Red = red;
            original(i, j)->Green = green;
            original(i, j)->Blue = blue;

            original(i+1, j)->Red = red;
            original(i+1, j)->Green = green;
            original(i+1, j)->Blue = blue;

            original(i, j+1)->Red = red;
            original(i, j+1)->Green = green;
            original(i, j+1)->Blue = blue;

            original(i+1, j+1)->Red = red;
            original(i+1, j+1)->Green = green;
            original(i+1, j+1)->Blue = blue;
        }
    }
	return original;
}
...
int main()
{
    ...
    // Individual tests
    output = makeImageSepia(test1, 0, 0, test1.TellWidth() - 100, test1.TellHeight() - 100);
