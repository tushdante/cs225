...
Triangle::Triangle(const RGBApixel& color, const Vector2& vertex1,
                   const Vector2& vertex2, const Vector2& vertex3)
: Shape(Vector2(0, 0), color),
...
int Triangle::area() const {
  ...
  const int area = static_cast<int>(0.5 * ab.crossProduct(ab));
  return area;
}
...
void Triangle::draw(BMP* canvas) const {
  /* Code taken from
  ...
  vector<double> x_small_medium = Line::linearInterpolation(*small, *medium);
...
void Triangle::set_center(const Vector2& center) {
  const Vector2 delta(center - this->center());
